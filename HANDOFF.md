...
doc is the practical "how do I run this and what's actually done" companion.
...
- **`sst_reasoning.py`** - now takes an optional `history` list (prior frames'
  ...
  lights -> cones -> blocked lane" behavior you described. `run_scenario.py`
  doesn't pass history (single-frame scenes), so it's unaffected.
- **`run_pipeline.py`** - new orchestrator tying the above together. Doesn't
...
- **`data/raw/sample_construction_drive.mp4`** - a synthetic test clip I
  stitched from `data/frames/construction_test/` (there was no raw video in
  the repo) so `run_pipeline.py` has something real to run against. It's 5
  source images held for a few frames each, not an actual dashcam clip - swap
  in a real one when you have one. A run against it is logged at
...
- **No retry/validation on vision output.** `vision_to_text.py` raises if the
  ...
  isn't guaranteed to always emit strict JSON - worth adding a retry-with-
  reprompt if it comes up.
...
- Everything already listed in `README.md` under "What Still Needs To Be
  ...
  human actions, confidence calibration. That list didn't change - I only
  touched the vision/temporal side.
